...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: solus.solus.stable ? ['log'] : ['verbose'],
  });
  ...
  if (solus.isSelfhosted && solus.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(solus.telemetry.token).track('selfhost-server-started', {
      version: solus.version,
    });
  }

...
solus client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/solusapp/solus
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/solusapp/solus/discussions/2840)
...
Also make sure you have built the `@solus/core` workspace before running E2E tests.
...
```shell
# there are `solus-local`, `solus-migration`, `solus-local`, `solus-prototype` e2e tests,
#   which are run under different situations.
cd tests/solus-local
yarn e2e
```

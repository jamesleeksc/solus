...
export class AppModuleBuilder {
  private readonly modules: solusModule[] = [];
  constructor(private readonly config: Config) {}

  use(...modules: solusModule[]): this {
    modules.forEach(m => {
      ...
      if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
          ...
          new Logger(name).warn(
            ...
            ...nonMetRequirements.map(config => `  solus.${config}`)
          );
          return;
        }
      }
...
function buildAppModule() {
  const factor = new AppModuleBuilder(solus);
  ...
  // plugin modules
  solus.plugins.enabled.forEach(name => {
